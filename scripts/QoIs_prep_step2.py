...
if withDst:
    ...
    res_folder = path + 'Maximum_storms/NoDst/%sT' % window_max
...
if withDst:
    ...
    # # ## let's calcualte the scaler first.
    if not os.path.exists(path + "scaler/scalerY_%s_%s_noDst.pkl" % (QoIs, window_max)):
        if not os.path.exists(path + "scaler/scalerX_%s_noDst.pkl" % window_max):
            year = 2005
            station_TargetYear_file = sorted([x for x in os.listdir(station_path) if bool(re.match(r'.*_%s\.pkl$' % year, x))])
            station_filepath_List = [station_path + x for x in station_TargetYear_file]
            train_Y = []
            ...
            ## fit scaler
            scalerX.fit(train_X.to_numpy())
            with open(path + "scaler/scalerX_%s_noDst.pkl" % window_max, 'wb') as f:
                pickle.dump(scalerX, f)
            scalerY.fit(train_Y)
            with open(path + "scaler/scalerY_%s_%s_noDst.pkl" % (QoIs, window_max), 'wb') as f:
                pickle.dump(scalerY, f)
        else:
            year = 2005
            station_TargetYear_file = sorted([x for x in os.listdir(station_path) if bool(re.match(r'.*_%s\.pkl$' % year, x))])
