...
# state_dict = torch.load(path_prefix+"/Paper_deltaB/data/train/model/paper/model_state_dBH_10T_1h_ahead.pth")
...
num_epochs = 50
num_samples = 10
tracking_folder = root_path+"/data/train/training_tracking/Maximum/%sHAhead/%s/%sT/" % (hour, QoIs, window_max)
model_earlystopping = root_path+"/data/train/model/model_state_%s_%sT_%sH.pth" % (QoIs, window_max, hour)
model_archive = root_path+"/data/train/model/Maximum/%sHAhead/%s/" % (hour, QoIs)
...
## we end up only having one group of data
j = 0
